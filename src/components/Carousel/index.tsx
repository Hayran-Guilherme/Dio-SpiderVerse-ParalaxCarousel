...
export default function Carousel({ heroes, activeId }: IProps) {
  ...
  // Sons de transição do carrossel
  const transitionAudio = useMemo(() => new Audio("/songs/transition.mp3"),[]);

  // Voz do personagem ao selecionar um herói
  ...
  // Atera o fundo da página de acordo com o herói selecionado
  useEffect(() => {
    ...
    // Remove a classe hero-page quando o componente é desmontado
    return () => {
  ...
  // Toca o som de transição e a voz do herói ao selecionar um herói
  useEffect(() => {
    if (!visibleItems) {
      return;
    }
  ...
  const handleDragEnd = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    const difPosition = endInteractionPosition - startInteractionPosition;

    const newPosition = difPosition > 0 ? -1 : 1;
